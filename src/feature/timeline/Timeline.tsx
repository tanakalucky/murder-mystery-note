        ...
        {dateGroups.map((dateGroup, dateIndex) => (
          <div key={dateIndex} className="mb-6 last:mb-0">
            <div className="bg-gray-100 px-4 py-2 rounded-t-md border-b-2 border-gray-300 mb-2 sticky top-0 z-10">
              <h3 className="font-semibold text-gray-800">{dateGroup.date}</h3>
            </div>
              ...
              {dateGroup.timeGroups.map((timeGroup, timeIndex) => (
                <div key={timeIndex} className="mb-4 last:mb-0">
                  <div className="flex items-center mb-2">
                    <div className="bg-gray-50 w-12 shrink-0 py-1 flex flex-col items-center justify-start border rounded-md">
                      <span className="font-medium text-gray-700 text-xs">{timeGroup.time}</span>
                    </div>
                    <div className="ml-2 h-0.5 flex-grow bg-gray-100"></div>
                  </div>
                    ...
                    {timeGroup.characterGroups.map((characterGroup, charIndex) => (
                      <div key={charIndex} className="mb-3 last:mb-0">
                        <div className="flex items-center mb-1">
                          ...
                          ></div>
                          <span className="text-xs font-medium">{characterGroup.character}</span>
                        </div>

                        <div className="space-y-2 ml-3">
                          {characterGroup.notes.map((note, noteIndex) => (
                            <Card key={noteIndex} className="bg-white shadow-sm py-0">
